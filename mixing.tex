\section{Horizontal and Vertical Mixing}
...
exist to diffuse on constant $z$ surfaces (\code{MIX\_GP\_UV},
\code{MIX\_GP\_TS}), constant {\sl in situ} density surfaces
(\code{MIX\_EPI\_UV}, \code{MIX\_EPI\_TS}), and constant potential
density surfaces ((\code{MIX\_ISO\_UV}, \code{MIX\_ISO\_TS})
...
$s$ surface. Here, \code{MIX\_EPI} represents both the epineutral and
...
\subsection{Timestepping vertical viscosity and diffusion}
\label{Vfric}
...
\label{vdiff3}
\end{multline}
where $k$ is used as the vertical level index.
