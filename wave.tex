...
There are \code{Python} scripts to create initial, climatology and
...
is now all coming from the output of a hydrology model (Hill and Beamer,
personal communication)---\code{RUNOFF} is not used here.

\subsubsection{NWGOA code chunks}
The \code{Apps/NWGOA} directory contains the \code{ana\_}
family of include files for setting the bottom drag, the horizontal
nudging coefficients and the passive tracers.
Doing a search for \code{NWGOA} shows up a few other bits of code.
The only such bit that is not shared by \code{ARCTIC} is in
\code{mod\_scalars.F}:
\begin{verbatim}
#ifdef NWGOA
...
\code{LBC} options are the same as in the \code{Arctic} for all open
boundaries. We did not need any nudging to climatology in this case.
...
The model writes voluminous information to standard out, as shown
for the \code{UPWELLING} case.  It also writes out NetCDF files for
restart, daily averages, stations and floats. In this case we also
saved hourly history output.
Plots can be made from any of these files; some examples are shown
in Fig.\ \ref{fnwgoa1}--\ref{fnwgoa2} .
