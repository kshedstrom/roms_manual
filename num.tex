...
the first radius of deformation (\cite{AL}).
\begin{figure}[htb]
...
accomplished in \code{Matlab} or \code{Python}; this
...
similar to that used in \cite{AL}; though their
...
\label{zeta3}
\end{equation}
\cite{SS2005} introduce a range of
...
integration of the density, as described in \cite{SS2005}.
...
one-dimensional (\cite{Leonard79}) and two-dimensional (\cite{Rasch94}
and \cite{SS98}). This scheme
...
$\frac{1}{8}$ while that in \cite{Rasch94} is $\frac{1}{6}$.
...
\cite{Jackett}.  It computes {\sl in situ} density as a
...
%errors when $|\nabla h|$ is large (\cite{Haney}; and 
%\cite{BH93}). Shchepetkin....
...
by (\cite{Chapman85}), assuming all outgoing signals leave
...
depths at the same horizontal location. \cite{Orlanski76}
...
\cite{Raymond84} have modified the scheme to account for
...
As described in (\cite{Marchesiello2001}), ROMS has an option for providing
