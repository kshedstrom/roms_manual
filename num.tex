...
to the closest interior value. This is the correct conditions for
tracers at a closed wall.

\subsubsection{Wall boundary condition}
ROMS now assumes a wall condition if no other boundary condition is
chosen. This is a zero gradient condition for tracers and zero flow
for the normal velocity. For tangential velocities, the wall is
treated as either no-slip or free-slip, depending on the value of
\code{gamma2} chosen by the user.
...
for (with the recommended \code{RADIATION_2D} option):
\begin{equation}
...
condition, requiring two timescales: the inflow timescale and the
outflow timescale. These timescales are provided in the input to
ROMS (\S\ref{ASCII_in}).

\subsubsection{Flather boundary condition}
For the normal component of the barotropic velocity, one option is
to radiate out at the speed of the external gravity waves (Flather,
\cite{Flather76}):
\begin{equation}
  \overline{u} = \overline{u}^{\rm ext} - \sqrt{\frac{g}{h}} \,
  (\zeta - \zeta^{\rm ext})
\end{equation}
...
The corresponding tangential boundary condition was developed by
Chapman (\cite{Chapman85}), equivalent to a reduced physics
boundary condition.
