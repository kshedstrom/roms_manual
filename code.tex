...
\begin{klist}
  ...
  \code{nl\_ocean.h}.
  \kitem{External} ROMS reads an ASCII file on startup. These are
  ...
  available options is in \code{cppdefs.h}.
...
\begin{klist}
  ...
  \kitem{main2d} or \code{main3d} solves the full
...
\begin{klist}
  ...
  \kitem{my25\_corstep} perfoms the corrector step for turbulent kinetic
...
files, especially for idealized problems.
   ...
   coefficients for nudging to a climatology.
     \kitem{ana\_pair}  Computes analytic air pressure.
...
step is done and in the monotonicity constraining algorithms.
  ...
  (\cite{Song98}).
    \kitem{ATM\_PRESS}  Define to impose atmospheric pressure onto the sea
  surface.
  \end{klist}
 ...
 cycle which is a function longitude, latitude and day-of-year.
  ...
  roughness (\cite{Oost_2002}).
    ...
    \kitem{DIURNAL\_SRFLUX} Define to impose shortwave radiation local
  diurnal cycle.
  \end{klist}
  ...
  conditions.
    ...
    \kitem{ANA\_PASSIVE} Define for analytic initial condtions for
  inert tracers.
 ...
 \kitem{ICE\_MODEL} Define to use ice component of the model (see
	...
	\kitem{ICE\_ALB\_EC92} Define for albedo computions from Ebert
